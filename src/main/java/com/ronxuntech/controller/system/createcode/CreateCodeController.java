package com.ronxuntech.controller.system.createcode;
...
import com.ronxuntech.service.system.createcode.CreateCodeManager;
import com.ronxuntech.util.AppUtil;
import com.ronxuntech.util.DateUtil;
import com.ronxuntech.util.DelAllFile;
import com.ronxuntech.util.FileDownload;
import com.ronxuntech.util.FileZip;
import com.ronxuntech.util.Freemarker;
...
//			System.out.println(pd.getString("field"+i));
		...
		String codepath = PathUtil.getClasspath()+"admin/ftl/code/";						//存放路径
		String apipath = codepath;
		String configpath = codepath;
		String jsppath = codepath;
		String ftlPath = "createCode";								//ftl路径
		String basedirpath = codepath;
		
		if(!basedir.trim().equals("") && (new File(basedir)).exists()) {
			String dir = (new File(basedir)).getAbsolutePath();
			basedirpath = dir;
			codepath = dir+"/src/main/java/com/ronxuntech/";
			configpath = dir + "/src/main/resources/";
			jsppath = dir + "/src/main/webapp/WEB-INF/";
			apipath = dir + "/src/main/java/api/ronxuntech/";
		} else {
			
		}
		
		/* 生成api调用 */
		Freemarker.printFile("apiControllerTemplate.ftl", root,"controller/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName+"ApiController.java", apipath, ftlPath);
		
		/*生成controller*/
		Freemarker.printFile("controllerTemplate.ftl", root, "controller/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName+"Controller.java", codepath, ftlPath);
		
		/*生成service*/
		Freemarker.printFile("serviceTemplate.ftl", root, "service/"+packageName+"/"+objectName.toLowerCase()+"/impl/"+objectName+"Service.java", codepath, ftlPath);
		
		/*生成manager*/
		Freemarker.printFile("managerTemplate.ftl", root, "service/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName+"Manager.java", codepath, ftlPath);

		/*生成mybatis xml*/
		Freemarker.printFile("mapperMysqlTemplate.ftl", root, "mybatis/"+packageName+"/"+objectName+"Mapper.xml", configpath, ftlPath);
...
//		
		/*生成SQL脚本*/
		Freemarker.printFile("mysql_SQL_Template.ftl", root, "mysql数据库脚本/"+tabletop+objectName.toUpperCase()+".sql", basedirpath+"/", ftlPath);
...
//		
		/*生成jsp页面*/
		Freemarker.printFile("jsp_list_Template.ftl", root, "jsp/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName.toLowerCase()+"_list.jsp", jsppath, ftlPath);
		Freemarker.printFile("jsp_edit_Template.ftl", root, "jsp/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName.toLowerCase()+"_edit.jsp", jsppath, ftlPath);
		
		/*生成说明文档*/
//		Freemarker.printFile("docTemplate.ftl", root, "部署说明.doc", configpath, ftlPath);
		
		//this.print("oracle_SQL_Template.ftl", root);  控制台打印
		
		if(!basedir.trim().equals("") && (new File(basedir)).exists()) {
			FileDownload.fileDownload(response, basedirpath+"/mysql数据库脚本/"+tabletop+objectName.toUpperCase()+".sql", "sql.sql");
		} else {
		/*生成的全部代码压缩成zip文件*/
