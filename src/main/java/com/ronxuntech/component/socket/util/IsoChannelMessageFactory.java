...
public class IsoChannelMessageFactory extends BaseController{
	...
	public String getInitBitMap128() {
		String initBitMap = "10000000" + "00000000" + "00000000" + "00000000" + "00000000" + "00000000" + "00000000"
	...
	public LinkedHashMap<String, Object> packBitMap(LinkedHashMap<String, Object> datafiledMap, String xmlPath) {
		// 读取xml文件，获取出应该返回的域。
		LinkedHashMap<Integer, Object> field = getReponseFiled(xmlPath);
		LinkedHashMap<String, Object> returnBit = new LinkedHashMap<String, Object>();
		...
		while (it1.hasNext()) {
			...
			if (fieldId == 7) {
				...
				returnBit.put("7", df0.format(date1));
				continue;
			}
			if (fieldId == 12) {
				SimpleDateFormat df1 = new SimpleDateFormat("HHmmss");
				returnBit.put("12", df1.format(date1));
				continue;
			}
			if (fieldId == 13) {
				SimpleDateFormat df2 = new SimpleDateFormat("MMdd");
				returnBit.put("13", df2.format(date1));
				continue;
			}
			// 将该返回的域，放入原来域的值
			returnBit.put("" + fieldId, datafiledMap.get("Bit " + fieldId));
		}
		return returnBit;
	}
	...
	public LinkedHashMap<String, Object> packNew(LinkedHashMap<String, Object> datafiledMap, String xmlPath,
			Message m) {
		// 得到返回的bitmap
		LinkedHashMap<String, Object> bitMap = packBitMap(datafiledMap, xmlPath);
		// 用来放回组装的报文
		LinkedHashMap<String, Object> returnMap = new LinkedHashMap<String, Object>();
		// 得到返回的报文
		byte[] returnBytes = pack(bitMap, m);
		// 将返回的域，以及值，重置
		m.setBitmap(bitMap);
		// 用来传递返回的报文。s
		returnMap.put("message", returnBytes);
		return returnMap;
	}
...
//						continue;
					...
					if (!isFixLen) {
						...
						if (fieldLen > maxLen) {
							System.out.println(
									"error:字段" + fieldId + "的数据定义长度的长度为" + defLen + "位,长度不能超过" + (10 * defLen1));
							return null;
						} else {
							// 将长度值组装入字段
