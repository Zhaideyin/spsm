...
public class TextRankKeyword
{
    ...
    public List getKeyword(String title, String content)
    {
        List<Term> termList = HanLP.segment(title + content);//分词
//        System.out.println(termList);
       ...
       // System.out.println(wordList);
        ...
        for(int i=0;i<nKeyword;i++){
            Map.Entry m=(Map.Entry)entryList.get(i);//['谈笑风生', 30]
            list.add(i,"['"+m.getKey()+"',"+((Float)m.getValue())*1000+"]");
        }
        return list;
    ...
    public static void main(String[] args)
    {
        ...
        arrayList=textRankKeyword.getKeyword("",content);
        for(int i=0;i<arrayList.size();i++){
            Map.Entry m=(Map.Entry)arrayList.get(i);
